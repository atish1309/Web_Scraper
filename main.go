...
func googleresultparsingfunc(response *http.Response, rank int) ([]SearchResult, error) {
	...
	//this id contains all req responses
	selected := doc.Find("div.g")
	rank += 1
...
func buildGoogleURLS(searchterm string, countryCode string, language_code string, noofpages, count int) ([]string, error) {
	thingstoscrape := []string{}
	searchterm = strings.Trim(searchterm, " ")
	searchterm = strings.Replace(searchterm, " ", "+", -1)
	//k:=googleDomains[countryCode]
	if k, found := googleDomains[countryCode]; found {
		for i := 0; i < noofpages; i++ {
			start := i * count
			scrapeURL := fmt.Sprintf("%s%s&num=%d&hl=%s&start=%d&filter=0", k, searchterm, count, language_code, start)
			thingstoscrape = append(thingstoscrape, scrapeURL)
		}
	} else {
		err := fmt.Errorf("country (%s) is currently not supported with this version of the scraper", countryCode)
		return nil, err
	}
	return thingstoscrape, nil
}

func scrapeClientREQ(searchURL string, proxyString interface{}) (*http.Response, error) {
	// no implementation for baseclient using proxystring
...
func GoogleScrape(searchterm string, countryCode string, language_code string, proxyString interface{}, noofpages, count, backoff int) ([]SearchResult, error) {
	resultCtr := 0
	results := []SearchResult{}
	gpages, err := buildGoogleURLS(searchterm, countryCode, language_code, noofpages, count)
	if err != nil {
		return nil, err
	}
	for _, page := range gpages {
		resp, err := scrapeClientREQ(page, proxyString)
		if err != nil {
			return nil, err
		}
		data, err := googleresultparsingfunc(resp, resultCtr)
		if err != nil {
			return nil, err
		}
		resultCtr += len(data)
...
func main() {
	res, err := GoogleScrape("atish", "com", "en", nil, 1, 30, 10)
	if err == nil {
		for _, res := range res {
			fmt.Println(res)
		}
